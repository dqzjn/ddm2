package cn.zmdx.locker.util.newCos;
...
{	
	...
	public static String uploadFileByStream(String url, Map<String, Object> data, Map<String, String> header, int timeOut, FileInputStream fileStream, int length) throws Exception{
		...
		try {
			...
			fileStream.read(bufferOut,0,length);
			ContentBody contentBody =  new ByteArrayBody(bufferOut, url.substring(url.lastIndexOf('/')+1));//new ByteArrayBody(bytes, fileName);
	...
	public static String uploadFileByStream(String url,
			HashMap<String, Object> data, HashMap<String, String> header,
			int timeOut, byte[] byteArray, int length) throws Exception {
		HttpClient httpClient = new DefaultHttpClient();
		httpClient.getParams().setParameter(CoreConnectionPNames.SO_TIMEOUT, timeOut); 
		try {
			HttpPost httpPost = new  HttpPost(url);
...
//			fileStream.skip(-length);
//			fileStream.read(bufferOut,0,length);
			ContentBody contentBody =  new ByteArrayBody(byteArray, url.substring(url.lastIndexOf('/')+1));//new ByteArrayBody(bytes, fileName);
			multipartEntity.addPart("fileContent", contentBody);
			httpPost.setEntity(multipartEntity);
			HttpResponse httpResponse = httpClient.execute(httpPost);
//			fileStream.close();
