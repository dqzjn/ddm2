...
public class CosCloud {
	 ...
	 * @param bucketName bucket名称
	 * @param remotePath 远程文件路径
	 * @param fileStream 上传文件数据流
	 * @return
	 * @throws Exception 
	 */
	public String uploadFile(String bucketName, String remotePath, FileInputStream fileStream) throws Exception{
		
		try {			
			String url = COSAPI_CGI_URL + appId + "/" + bucketName + encodeRemotePath(remotePath);
			int length=fileStream.available();
			String sha1 = HMACSHA1.getFileSha1(fileStream);
			HashMap<String, Object> data = new HashMap<String, Object>();
			data.put("op", "upload");
			data.put("sha", sha1);
			long expired = System.currentTimeMillis() / 1000 + 60;
			...
			HashMap<String, String> header = new HashMap<String, String>();
			header.put("Authorization", sign);
			return Request.uploadFileByStream(url, data, header, timeOut, fileStream,length);			
		} catch (Exception e) {
			throw e;
		}
	}
	...
	public String uploadFile(String bucketName, String remotePath,
		...
		try {			
			...
			String sha1 = HMACSHA1.getFileSha1(byteArray);
			HashMap<String, Object> data = new HashMap<String, Object>();
			data.put("op", "upload");
			data.put("sha", sha1);
			long expired = System.currentTimeMillis() / 1000 + 60;
			...
			HashMap<String, String> header = new HashMap<String, String>();
			header.put("Authorization", sign);
			return Request.uploadFileByStream(url, data, header, timeOut, byteArray,length);
		} catch (Exception e) {
			throw e;
		}
	}
